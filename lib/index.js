 ...
 */
async function serializeRequest(options, defaults, resolveImage, signal, wireModel) {
  const messages = [];
...
function modelInfo(provider, model, fallbackEnabled) {
  return {
    provider,
    id: model.id,
    name: model.name ?? model.id,
    ...(model.description === undefined ? {} : { description: model.description }),
    inputModalities: model.inputModalities ?? (fallbackEnabled ? ["text", "image"] : ["text"]),
  };
}

/**
 * Decide the model id that actually goes on the wire. When the request
 * carries image content and the selected route does not declare image input,
 * reroute the request to the configured multimodal fallback model (default
 * `qwen3.8-max`). The caller-visible model name stays unchanged — only the
 * wire id changes, so usage records still attribute the call to the route
 * the user picked. An empty `multiModalFallbackModel` disables rerouting.
 */
function resolveWireModel(connection, options) {
  const fallback = connection.multiModalFallbackModel;
  if (fallback === "" || options.model === fallback) return options.model;
  const containsImage = options.messages.some((message) => contentHasImage(message.content));
  if (!containsImage) return options.model;
  const imageCapable = connection.models.some(
    (entry) => entry.id === options.model && entry.inputModalities !== undefined && entry.inputModalities.includes("image"),
  );
  return imageCapable ? options.model : fallback;
}
...
class QwenAdapter extends LlmAdapter {
  ...
  listModels(provider) {
    ...
    return Promise.resolve(connection.models.map((model) => modelInfo(provider, model, connection.multiModalFallbackModel !== "")));
  }
  resolveModel(provider, model, _signal) {
    const connection = this.config.options();
    const configured = connection.models.find((entry) => entry.id === model);
    const fallbackEnabled = connection.multiModalFallbackModel !== "";
    ...
    return Promise.resolve({
      ...(configured === undefined
        ? {
            ...
            id: model,
            name: model,
            inputModalities: fallbackEnabled ? ["text", "image"] : ["text"],
          }
        : modelInfo(provider, configured, fallbackEnabled)),
  ...
  async *request(options, signal) {
    ...
    const body = await serializeRequest(options, connection.defaults, this.config.resolveImage, signal, resolveWireModel(connection, options));
    const headers = {
...
const DEFAULT_MULTIMODAL_FALLBACK_MODEL = "qwen3.8-max";
// Per-model advisory context windows (official specs, not gateway-detected):
// qwen3.5+/3.6/3.7/3.8 chat and DeepSeek V4 are native 1M; Qwen3 Coder / VL
// and Kimi K2 are 256K; DeepSeek V3.2 and GLM-5.x are 128K.
const MEGABYTE_CONTEXT_WINDOW = 1048576;
const QUARTER_MEBIBYTE_CONTEXT_WINDOW = 262144;
...
const DEFAULT_MODELS = [
  { id: "qwen3.8-max", name: "Qwen3.8 Max", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS, inputModalities: ["text", "image"] },
  { id: "qwen3.7-max", name: "Qwen3.7 Max", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS, inputModalities: ["text", "image"] },
  { id: "qwen3.7-flash", name: "Qwen3.7 Flash", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS, inputModalities: ["text", "image"] },
  { id: "qwen3.6-plus", name: "Qwen3.6 Plus", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS, inputModalities: ["text", "image"] },
  { id: "qwen3.6-flash", name: "Qwen3.6 Flash", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS, inputModalities: ["text", "image"] },
  { id: "qwen3-coder-plus", name: "Qwen3 Coder Plus", contextWindow: QUARTER_MEBIBYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS },
  { id: "qwen3-coder-flash", name: "Qwen3 Coder Flash", contextWindow: QUARTER_MEBIBYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS },
  { id: "qwen3-vl-plus", name: "Qwen3 VL Plus", contextWindow: QUARTER_MEBIBYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS, inputModalities: ["text", "image"] },
  { id: "qwen3-vl-flash", name: "Qwen3 VL Flash", contextWindow: QUARTER_MEBIBYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS, inputModalities: ["text", "image"] },
  // Third-party models served through the DashScope gateway.
  { id: "deepseek-v4-flash", name: "DeepSeek V4 Flash (via Qwen)", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS },
  { id: "deepseek-v4-flash-0731", name: "DeepSeek V4 Flash 0731 正式版 (via Qwen)", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS },
  { id: "deepseek-v4-pro", name: "DeepSeek V4 Pro (via Qwen)", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS },
  { id: "deepseek-v4-pro-0813", name: "DeepSeek V4 Pro 0813 正式版 (via Qwen)", contextWindow: MEGABYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS },
  ...
  { id: "kimi-k2.7-code", name: "Kimi K2.7 Code (via Qwen)", contextWindow: QUARTER_MEBIBYTE_CONTEXT_WINDOW, maxTokens: DEFAULT_MAX_TOKENS },
];
...
const Config = z.object({
  ...
  // When a request carries image content but the selected route is not a
  // multimodal model, that request is transparently routed to this model id.
  // Empty string disables the reroute (image requests then fail with the
  // stock "model does not accept image input" error).
  multiModalFallbackModel: z.string().default(DEFAULT_MULTIMODAL_FALLBACK_MODEL),
...
function resolveAdapterOptions(config, environment) {
  ...
  return {
    ...
    multiModalFallbackModel: config.multiModalFallbackModel?.trim() ?? DEFAULT_MULTIMODAL_FALLBACK_MODEL,
    streamIdleTimeoutMs,
