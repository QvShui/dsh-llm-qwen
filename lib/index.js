...
/**
 * Resolve one legal thinking/effort pair. Qwen3 thinking is a boolean
 * (`enable_thinking`); the harness reasoning-effort vocabulary maps onto it
 * as `off`/`on`. Session titles never think (fast and cheap).
 */
function resolveThinking(options, defaults) {
  ...
  return { enableThinking: effort === "on" };
}
...
const ON_REASONING_EFFORT = ReasoningEffortId("on");
const REASONING_EFFORTS = [
  { id: OFF_REASONING_EFFORT, name: "Off" },
  { id: ON_REASONING_EFFORT, name: "On" },
];
      ...
      ...(connection.defaults.thinking === "disabled"
        ...
        : {
            reasoning: {
              ...
              defaultEffort: connection.defaults.reasoningEffort === "off" ? OFF_REASONING_EFFORT : ON_REASONING_EFFORT,
            },
          }),
    });
  }
...
const Config = z.object({
  ...
  reasoningEffort: z.union(["off", "on"]),
